...
export function bootThemeFx(app) {
  ...
  function applyTheme({ snow, reindeer, preset = "classic", intensity = 1 }) {
    ...
    currentIntensity = Number(intensity) || 1.1;
    snowCtl?.setIntensity?.(currentIntensity);
    snowCtl?.setEnabled?.(!!snow);
  ...
  function startReindeer() {
    ...
    const base = 3200;
    const every = Math.max(900, Math.floor(base / (currentIntensity || 1)));
    reindeerTimer = setInterval(spawnReindeerWave, every);
  }
  ...
  function spawnReindeerWave() {
    ...
    if (!reindeerLayer) return;
    const emojis = ["🦌", "🦌", "🦌", "🛷", "🦌"];
    const count = Math.max(3, Math.round(6 * (currentIntensity || 1)));
...
function snowInit() {
  ...
  function makeFlake() {
    ...
    return {
      x: rand(0, w),
      y: rand(-h, 0),
      r: rand(1.0, 4.0) * (0.9 + k * 0.08),
      vy: rand(0.7, 2.4) * (0.85 + k * 0.12),
      vx: rand(-0.6, 0.8),
      sway: rand(0.002, 0.014) * (0.85 + k * 0.1),
      phase: rand(0, Math.PI * 2)
    };
  }

  function refill() {
    const target = Math.max(40, Math.round(BASE_FLAKES * (intensity || 1)));
    flakes.length = 0;
    for (let i = 0; i < target; i++) flakes.push(makeFlake());
  }
...
// Pills: define intensidade (tímido / normal / super natalino)
pills.forEach(btn => {
  btn.addEventListener("click", () => {
    const v = Number(btn.getAttribute("data-int")) || 1.1;
    if (optIntensity) optIntensity.value = String(v);
    markPills(v);
    applyNow();
  });
});
