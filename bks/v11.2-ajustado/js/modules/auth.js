...
// v8.2: avatar é compartilhado (ranking) via campo `avatar` (dataURL) no Firestore.
...
export function bootAuth(app) {
  ...
  if (!auth) {
    console.warn("[auth] Firebase Auth (app.firebase.auth) não disponível.");
    return;
  }
...
function isValidEmail(e){
  ...
  app.auth = {
    ...
    // abre direto na aba "Criar conta" (útil para outros módulos como o chat)
    openSignup: () => openAuthGate({ force: false, defaultTab: "signup" }),
  ...
  async function fetchOrCreateProfile(user){
    ...
    return { uid: user.uid, email, name: nameGuess, sector: "", avatar: "", needsCompletion: true };
  }
...
onClick: () => {
        ...
        <div class="account-panel">
          <div class="account-avatar">
            <div class="avatar-preview">
              ${avatar ? `<img id="accountAvatarImg" src="${escapeHtml(avatar)}" alt="" referrerpolicy="no-referrer"/>` : `<div id="accountAvatarImg" class="avatar-fallback">${escapeHtml((name||'U')[0]||'U')}</div>`}
            </div>
  ...
  // -----------------------------
  // Avatar (foto) — v8.2
  // ✅ Sem Storage: avatar é salvo como dataURL (base64) no Firestore
  //   - users/{uid}.avatar (somente dono lê/escreve)
  //   - rankingByEmail/{emailHash}.avatar (leitura pública)
  // ⚠️ Suas rules limitam avatar <= 200000 chars (≈200KB). Vamos comprimir até caber.
  // -----------------------------
  function wireAccountAvatarUI(){
    ...
    if (fileEl){
      fileEl.addEventListener('change', async () => {
        ...
        if (!f) return;
        try{
          const dataUrl = await imageFileToDataUrlCapped(f, 200000);
          await setMyAvatar(dataUrl);
          setStatus('Foto atualizada ✅');
          rerenderAccountAvatar(dataUrl);
        }catch(e){
          console.warn('[auth] falha ao enviar foto', e);
          setStatus('Não foi possível enviar a foto. Tente uma imagem menor (ou recorte) e tente novamente.');
        }
  ...
  function rerenderAccountAvatar(dataUrl){
    const name = (currentProfile?.name || currentUser?.displayName || 'U');
    const first = (String(name||'U')[0] || 'U');
    const wrap = document.querySelector('.avatar-preview');
    if (!wrap) return;
    if (dataUrl){
      wrap.innerHTML = `<img id="accountAvatarImg" src="${escapeHtml(dataUrl)}" alt="" referrerpolicy="no-referrer"/>`;
    } else {
      wrap.innerHTML = `<div id="accountAvatarImg" class="avatar-fallback">${escapeHtml(first)}</div>`;
    }
  }
  ...
  async function setMyAvatar(dataUrl){
    ...
    await setDoc(refU, { avatar: String(dataUrl||''), updatedAt: serverTimestamp() }, { merge: true });
    if (currentProfile) currentProfile.avatar = String(dataUrl||'');
    try { await app.ranking?.setMyAvatar?.(String(dataUrl||'')); } catch {}
  }
  ...
  async function removeMyAvatar(){
    ...
    if (!u) throw new Error('not-auth');
    const refU = doc(db, 'users', u.uid);
    await setDoc(refU, { avatar: '', updatedAt: serverTimestamp() }, { merge: true });
    if (currentProfile) currentProfile.avatar = '';
    try { await app.ranking?.setMyAvatar?.(''); } catch {}
  }

  function imageFileToDataUrlCapped(file, maxChars = 200000){
    // Faz crop quadrado + compressão JPEG progressiva até caber nas rules.
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          try{
            ...
            const out = Math.min(256, s);
            canvas.width = out;
            canvas.height = out;
            ctx.drawImage(img, sx, sy, s, s, 0, 0, out, out);

            // tenta qualidades decrescentes
            const qualities = [0.82, 0.75, 0.68, 0.6, 0.52, 0.45, 0.38, 0.32];
            for (const q of qualities){
              const dataUrl = canvas.toDataURL('image/jpeg', q);
              if (String(dataUrl).length <= maxChars) return resolve(dataUrl);
            }

            // fallback: reduz ainda mais tamanho final
            canvas.width = 192;
            canvas.height = 192;
            ctx.clearRect(0,0,192,192);
            ctx.drawImage(img, sx, sy, s, s, 0, 0, 192, 192);
            const dataUrl = canvas.toDataURL('image/jpeg', 0.32);
            if (String(dataUrl).length <= maxChars) return resolve(dataUrl);
  ...
  // =============================
  function openAuthGate({ force, defaultTab } = { force: false, defaultTab: "login" }) {
    ...
    afterModalPaint(() => {
      ...
      // permite abrir direto na aba de cadastro (ou voltar para login)
      if (defaultTab === "signup") {
        setGateTab("signup");
      } else {
        setGateTab("login");
      }
      // setGateTab já faz o foco correto
    });
  }
...
// Compat: versões anteriores usavam setStatus(...)
function setStatus(msg){
  const s = String(msg || "");
  const ok = s.includes("✅") || s.toLowerCase().includes("sucesso") || s.toLowerCase().includes("atualizada");
  showStatus(s, ok ? "ok" : "error");
}
