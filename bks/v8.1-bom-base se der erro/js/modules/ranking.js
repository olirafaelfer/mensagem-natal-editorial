...
export function bootRanking(app){
  ...
  // V8: avatar é compartilhado via campo avatarUrl no rankingByEmail.
  ...
  async function upsertForChallenge(ch, metrics){
    ...
    // Foto (compartilhada): vem do perfil (users/{uid}.photoURL) e é copiada para o ranking.
    ...
    const avatarUrl = String(profile?.photoURL || u.photoURL || prev.avatarUrl || '').trim();
    ...
    const payload = {
      ...
      avatarUrl: avatarUrl ? String(avatarUrl).slice(0, 600) : '',
    ...
    return `
      <div class="rank-list">
        ...
        ${rows.map((d,i)=>{
          ...
          const av = (d && typeof d.avatarUrl === 'string' && d.avatarUrl) ? d.avatarUrl : '';
          const avHtml = av
  ...
  app.ranking = {
    ...
    setMyAvatarUrl: async (avatarUrl) => {
      try{
        if (!app.auth?.isLogged?.()) return;
        const u = fb.auth.currentUser;
        const email = (u?.email || '').trim().toLowerCase();
        ...
        const emailHash = await sha256Hex(email);
        const ref = fb.doc(fb.db, 'rankingByEmail', emailHash);
        await fb.setDoc(ref, { avatarUrl: String(avatarUrl||''), updatedAt: fb.serverTimestamp() }, { merge: true });
      }catch(e){
        console.warn('[ranking] setMyAvatarUrl falhou', e);
      }
    },
