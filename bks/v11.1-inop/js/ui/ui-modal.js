...
export function bootModal(app){
  ...
  // API do modal
  // =========================
  function openModal({ title, bodyHTML, buttons = [], dismissible = false, size = '' }){
    ...
    // size class (ex.: 'xl' para conteúdos grandes como o cartão)
    try{
      modal?.classList?.remove('modal-xl');
      if (String(size).toLowerCase() === 'xl') modal?.classList?.add('modal-xl');
    }catch{}
