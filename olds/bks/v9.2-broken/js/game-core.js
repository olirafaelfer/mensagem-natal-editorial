    ...
    b.addEventListener("click", () => {
      ...
      openModal?.({
        ...
        body: snap ? (() => {
          ...
          const fixes = Array.isArray(snap.fixes) ? snap.fixes : [];
          const rules = (fixes.length ? fixes : (snap.rules||[])).map((r, idx) => {
            const isFix = !!(r && (r.kind || r.before || r.after) && !r.wrong);
            if (isFix){
              const wrong = (r.before ?? "").toString();
              const correctRaw = (r.after ?? "").toString();
              const correct = (correctRaw === "") ? "(remover)" : correctRaw;
              const why = (r.reason || "").toString();
              return `
          ...
          return `
            <div class="final-review">
              <p class="muted" style="margin:0 0 8px">Seu texto:</p>
              <div class="final-review-box">${escapeHtml(String((snap.after ?? snap.before) || ""))}</div>
...
if (dom.finalMissionSpecialBtn){
  ...
  function openTutorialGate(){
    ...
    if (p?.c1?.done){
      openModal({
        title:"📘 Tutorial",
        bodyHTML:`<p><b>O tutorial só pode ser feito antes de realizar o desafio 1.</b></p>`,
...
// =========================
  ...
  function renderMessage(isTutorial=false){
    ...
    // PONTUAÇÃO: modo "somente remover" — todas as vírgulas clicáveis
    if (lvl && lvl.punctuationOnly){
      const txt = String(engine.currentText || "");
      let i = 0;
      for (const ch of txt){
        if (ch === "," ){
          const s = document.createElement("span");
          const locked = engine.lockedPositions?.has(i);
          s.className = locked ? "token token-wrong locked" : "token token-error";
          s.textContent = ch;
          s.dataset.kind = "error";
          s.dataset.pos = String(i);
          if (!locked){
            s.addEventListener("click", () => onTokenClick(s, null));
          }
          dom.messageArea.appendChild(s);
        } else {
          const t = document.createTextNode(ch);
          dom.messageArea.appendChild(t);
        }
        i++;
      }
      return;
    ...
    while (pos < text.length){
      ...
      if (!best){
        appendPlain(text.slice(pos), pos);
        break;
      }
      if (best.index > pos) appendPlain(text.slice(pos, best.index), pos);
      ...
      if (bestRule?.clickTokenOnly && bestRule?.token && best.text.includes(bestRule.token)){
        ...
        for (const ch of best.text){
          if (ch === tok){
            if (buf){ appendPlain(buf, best.index + (posLocal - buf.length)); buf = ""; }
            const cspan = document.createElement("span");
            cspan.className = tokenClass;
            cspan.textContent = tok;
            cspan.dataset.kind = "error";
            cspan.dataset.pos = String(best.index + (posLocal||0));
        ...
        if (buf) appendPlain(buf, best.index + (posLocal - buf.length));
      } else {
        const span = document.createElement("span");
        span.className = tokenClass;
        span.textContent = best.text;
        span.dataset.kind = "error";
        span.dataset.pos = String(best.index);
    ...
    function appendPlain(seg, startIndex){
      let local = startIndex ?? 0;
      // divide em espaços + tokens, mas pontuação vira 1-char para permitir cliques precisos
      ...
      for (const part of parts){
        ...
        // Se for apenas pontuação, cria um span por caractere
        if (/^[,.;:!?]+$/.test(part)){
          for (const ch of part){
            const s = document.createElement("span");
            s.className = "token plain punct";
            s.textContent = ch;
            s.dataset.kind = "plain";
            s.dataset.pos = String(local);
...
// =========================
  ...
  function findRemovalRuleForPos(pos, ch){
    try{
      const st = engine.getState?.();
      const lvl = st?.level;
      const text = String(engine.currentText || "");
      const rules = (lvl?.rules || []).filter(r => r && !engine.fixedRuleIds.has(r.id) && String(r.correct ?? "") === "" );
      if (!rules.length) return null;
      for (const r of rules){
        // only consider rules that intend removal of this punctuation token
        const tok = String(r.token || "");
        if (tok && tok !== ch) continue;
        const re = new RegExp(r.wrong.source, r.wrong.flags.includes("g") ? r.wrong.flags : (r.wrong.flags+"g"));
        let m;
        while ((m = re.exec(text))){
          const start = m.index;
          const end = start + m[0].length;
          if (pos >= start && pos < end && String(m[0]).includes(ch)){
            return r;
          }
  ...
  function applyRemovalAtPos(el, pos, ch){
    if (engine.lockedPositions?.has(pos)) { toast?.("⚠️ Esta vírgula já foi removida."); return; }
    const rule = findRemovalRuleForPos(pos, ch);
    const text = String(engine.currentText || "");
    // confirmação
    openModal({
      title: "Remover pontuação?",
      bodyHTML: `<p>Você deseja remover <b>${escapeHtml(ch)}</b>?</p>`,
      buttons:[
        {label:"Cancelar", variant:"ghost", onClick: closeModal},
        {label:"Remover", onClick: () => {
          ...
          const text = String(engine.currentText || "");
          let delta = 0;
          // remove exatamente no índice clicado (mais confiável do que regex/contexto)
          if (pos >= 0 && pos < text.length && text[pos] === ch){
            engine.currentText = text.slice(0,pos) + text.slice(pos+1);
            try{ engine.lockedPositions?.add(pos); }catch(e){}
          } else {
            // fallback: remove a primeira ocorrência do caractere
            engine.currentText = text.replace(ch, "");
          }

          if (rule){
            delta = engine.applyCorrect(rule.id);
            engine.logFix?.({ kind:"click-remove", challenge: engine.challenge, levelIndex: engine.levelIndex,
              label: rule.label||"", before: ch, after:"", reason: (rule.reason||rule.hint||"") });
          } else {
            delta = engine.penalizeMisclick();
            engine.logFix?.({ kind:"misclick-remove", challenge: engine.challenge, levelIndex: engine.levelIndex,
              label: "pontuação", before: ch, after:"", reason: "Remoção indevida" });
          }

          // feedback
          scoreFloat(delta, el);
          toast?.(rule ? "✅ Pontuação removida" : "⚠️ Remoção incorreta");
...
function onPlainClick(el){
    ...
    if (ch === "," || ch === ";" || ch === ":" || ch === "!" || ch === "?" || ch === "."){
      const pos = parseInt(el?.dataset?.pos || "-1", 10);
      if (pos >= 0) return applyRemovalAtPos(el, pos, ch);
    }
    ...
    openModal({
      ...
      buttons:[
        ...
        {label:"Confirmar", onClick: () => {
          ...
          try{
            if (el){
              el.classList.add("token-wrong","token-locked");
  ...
  function onTokenClick(el, rule){
    ...
    const lvl = st?.level;
    if (engine.isDone?.() && engine.challenge!==0) return;
    const ch0 = (el?.textContent || "");
    if ((ch0 === "," || ch0 === ";" || ch0 === ":" || ch0 === "." || ch0 === "!" || ch0 === "?") && engine.challenge!==0){
      const pos = parseInt(el?.dataset?.pos || "-1", 10);
      if (pos >= 0) return applyRemovalAtPos(el, pos, ch0);
    }
    ...
    if (engine.challenge !== 0 && rule?.fast === true){
      try{
        engine.logFix({ kind:"manual", challenge: engine.challenge, levelIndex: engine.levelIndex, label: rule.label||"", before: el?.textContent||"", after: (rule.correct ?? ""), reason: (rule.hint||rule.reason||"") });
    ...
    openModal({
      ...
      buttons:[
        ...
        {label: isRemove ? "Remover" : "Confirmar", onClick: () => {
          ...
          if (!ok){
            // marca como errado e bloqueia (não permite tentar de novo)
            try{
              if (tokenEl){
                tokenEl.classList.add("wrong","blocked","token-wrong","token-locked");
            ...
            scoreFloat(delta, tokenEl || dom.nextLevelBtn);

            openModal({
              title:"❌ Correção incorreta",
              bodyHTML:`<p>Essa correção não está certa. Você perdeu <b>${Math.abs(app.data.SCORE_RULES.wrong)}</b> pontos.</p>
                       <p class="muted">Este trecho ficou <b>bloqueado</b> para evitar penalidades repetidas.</p>`,
              buttons:[ {label:"Entendi", variant:"primary", onClick: closeModal} ],
              dismissible: true
            });
            updateHUD();
  ...
  function renderFinalMessages(){
    ...
    function buildJustificativasHTML(rules){
      const items = [];
      for (const r of (rules||[])){
        const wrong = (r.wrongStr || r.wrong || "").toString();
        const correct = (r.correct ?? "").toString();
        const why = (r.justification || r.hint || r.reason || "").toString();

        if (!wrong && !correct && !why) continue;

        items.push(`
          <div class="why-row">
            <div class="why-label">${esc(r.label || "Correção")}</div>
            <div class="why-body">
              ${wrong ? `<div><b>Palavra/trecho errado:</b> ${esc(wrong)}</div>` : ``}
              <div><b>Correção:</b> ${esc(correct === "" ? "(remover)" : correct)}</div>
              <div class="muted"><b>Justificativa:</b> ${esc(why || "(a definir)")}</div>
            </div>
          </div>
        `);
      }
      if (!items.length) return `<p class="muted">(Sem justificativas cadastradas para esta atividade)</p>`;
      return `<div class="why-list">${items.join("")}</div>`;
    }

    function diffUserVsPerfect(user, perfect){
      // marca apenas os tokens diferentes, sem “puxar” vizinhos
      const ua = tokenize(user);
      const up = tokenize(perfect);
      if (Math.abs(ua.length - up.length) > 40) return esc(user);

      const out = [];
      const n = Math.max(ua.length, up.length);
      for (let i=0;i<n;i++){
        const a = ua[i] ?? "";
        const p = up[i] ?? "";
        if (a === p){
          ...
          // token do usuário (o que ele deixou escrito)
          if (a) out.push(`<span class="diff-wrong" title="Clique para ver dica">${esc(a)}</span>`);
          // se o token correto existe e o usuário “pulou”, mostramos em verde como referência
          if (!a && p) out.push(`<span class="diff-correct" title="Token correto">${esc(p)}</span>`);
        }
      }
      return out.join("");
    }
    ...
    for (let i=0;i<3;i++){
      ...
      const before = String(snap.before || "");
      const user = String(snap.user || snap.after || "");
      const perfect = applyRules(before, snap.rules || []);

      const userHTML = snap.punctuationOnly ? esc(user) : diffUserVsPerfect(user, perfect);

      box.innerHTML = `
        <div class="final3-wrap">
          <div class="final3-col">
            <div class="final3-title">Texto inicial</div>
            <div class="final3-text">${esc(before)}</div>
          </div>
          <div class="final3-col">
            <div class="final3-title">Seu texto</div>
            <div class="final3-text">${userHTML}</div>
          </div>
          <div class="final3-col">
            <div class="final3-title">Texto correto</div>
            <div class="final3-text">${esc(perfect)}</div>
          </div>
        </div>
        <div class="final-why">
          <div class="final3-title">Justificativas</div>
          ${buildJustificativasHTML(snap.rules || [])}
        </div>
      `;

      box.querySelectorAll(".diff-wrong,.diff-correct").forEach(el=>{
        el.addEventListener("click", ()=>{
          openModal({
            title: "🔎 Detalhe",
            bodyHTML: `<p><b>${esc(el.textContent || "")}</b></p><p class="muted">A justificativa completa está na seção “Justificativas”.</p>`,
            buttons:[{label:"Ok", onClick: closeModal}],
            dismissible:true
          });
        });
    ...
    try{
      ...
      if (engine.challenge !== 0 && stNow?.level){
        levelSnapshots[engine.levelIndex] = {
          ...
          user: String(engine.currentText || ""),
          // regras originais do nível (com wrong/correct/hint/justificativa)
          punctuationOnly: !!stNow.level.punctuationOnly,
          rules: Array.isArray(stNow.level.rules) ? JSON.parse(JSON.stringify(stNow.level.rules)) : [],
          // log das ações do usuário nessa atividade
          fixes: (engine.getFixLog?.() || []).filter(f => f && f.levelIndex === engine.levelIndex && f.challenge === engine.challenge),
        };
      }
    }catch(e){ /* noop */ }
