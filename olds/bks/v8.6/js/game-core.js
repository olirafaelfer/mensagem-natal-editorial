      ...
      openModal?.({
        ...
        body: snap ? (() => {
          ...
          const rules = (snap.rules||[]).map((r, idx) => {
            const wrong = (r.labelWrong || (r.wrong?.source ?? r.wrong) || "").toString();
            const correct = (r.labelCorrect || r.correct || "").toString();
            const why = (r.why || r.reason || r.explain || "").toString();
            return `
...
// =========================
  ...
  function renderMessage(isTutorial=false){
    ...
    while (pos < text.length){
      ...
      // ✅ Para regras que removem vírgula (pontuação), renderizamos APENAS a vírgula como clicável.
      // Isso melhora a UX: o usuário clica na vírgula, não no trecho inteiro.
      const isCommaRemoval = typeof bestRule?.wrong === "string"
        && typeof bestRule?.correct === "string"
        && bestRule.wrong.includes(",")
        && bestRule.correct === bestRule.wrong.replaceAll(",", "");

      if (isCommaRemoval && best.text.includes(",")) {
        let buf = "";
