...
// V8: avatar é compartilhado (ranking) via Firebase Storage + Firestore.
...
function isValidEmail(e){
  ...
  async function fetchOrCreateProfile(user){
    ...
    return { uid: user.uid, email, name: nameGuess, sector: "", photoURL: user.photoURL || "", needsCompletion: true };
  }
...
onClick: () => {
  ...
  // -----------------------------
  // Avatar (foto) — V8
  // ✅ Upload real via Firebase Storage (avatars/{uid}.jpg)
  // ✅ URL pública salva em users/{uid}.photoURL e também em rankingByEmail.avatarUrl
  // ✅ Ícones (emoji) continuam funcionando via dataURL (pequeno)
  // -----------------------------
    ...
    if (fileEl){
      fileEl.addEventListener('change', async () => {
        ...
        try{
          ...
          await setMyPhotoURL(url);
          setStatus('Foto atualizada ✅');
          rerenderAccountAvatar(url);
        }catch(e){
          console.warn('[auth] falha ao enviar foto', e);
          setStatus('Não foi possível enviar a foto. Verifique as rules do Storage e tente outra imagem.');
        }
  ...
  function rerenderAccountAvatar(url){
    ...
    if (!wrap) return;
    if (url){
      wrap.innerHTML = `<img id="accountAvatarImg" src="${escapeHtml(url)}" alt="" referrerpolicy="no-referrer"/>`;
    } else {
  ...
  async function uploadAvatarToStorage(file){
    ...
    const blob = await imageFileToJpegBlob(file, 256, 0.86);
  ...
  async function setMyPhotoURL(url){
    const u = auth.currentUser;
    if (!u) throw new Error('not-auth');
    const refU = doc(db, 'users', u.uid);
    await setDoc(refU, { photoURL: String(url||''), photoUpdatedAt: serverTimestamp() }, { merge: true });
    // Atualiza cache em memória
    if (currentProfile) currentProfile.photoURL = String(url||'');
    // Atualiza ranking global (para todo mundo ver)
    try { await app.ranking?.setMyAvatarUrl?.(String(url||'')); } catch {}
  }

  async function removeMyPhotoURL(){
    const u = auth.currentUser;
    if (!u) throw new Error('not-auth');
    // tenta deletar o arquivo padrão (não falha se não existir)
    try{
      if (storage){
        const r = storageRef(storage, `avatars/${u.uid}.jpg`);
        await deleteObject(r);
      }
    }catch{}
    const refU = doc(db, 'users', u.uid);
    await setDoc(refU, { photoURL: '', photoUpdatedAt: serverTimestamp() }, { merge: true });
    if (currentProfile) currentProfile.photoURL = '';
    try { await app.ranking?.setMyAvatarUrl?.(''); } catch {}
  }

  function imageFileToJpegBlob(file, maxSizePx = 256, quality = 0.86){
