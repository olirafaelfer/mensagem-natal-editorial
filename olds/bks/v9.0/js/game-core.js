...
if (dom.finalMissionSpecialBtn){
  ...
  function openTutorialGate(){
    const p = getProgress();
    if (p?.c1?.done){ toast?.("O tutorial só pode ser feito antes de realizar o desafio 1."); return; }
    openModal({
      title: '📘 Tutorial',
    ...
    openModal({
      ...
      buttons:[
        ...
        {label:"Remover", onClick: () => {
          ...
          if (rule){
            // substitui a ocorrência correspondente ao clique (match que contém pos)
            try{
              const re = new RegExp(rule.wrong.source, rule.wrong.flags.includes("g") ? rule.wrong.flags : (rule.wrong.flags+"g"));
              let out = "";
              let last = 0;
              ...
              while ((m = re.exec(text))){
                const start = m.index;
                const end = start + m[0].length;
                if (!did && pos >= start && pos < end && String(m[0]).includes(ch)){
                  out += text.slice(last, start) + String(rule.correct ?? "");
                  last = end;
                  did = true;
                }
                if (m[0].length === 0) break;
              ...
              out += text.slice(last);
              if (did) engine.currentText = out;
              else {
                // fallback: remove char at pos
                engine.currentText = text.slice(0,pos) + text.slice(pos+1);
              }
            }catch(e){
              engine.currentText = text.replace(rule.wrong, rule.correct);
            }
            delta = engine.applyCorrect(rule.id);
            engine.logFix?.({ kind:"click-remove", challenge: engine.challenge, levelIndex: engine.levelIndex,
              label: rule.label||"", before: ch, after:"", reason: rule.reason||"" });
          } else {
            // remoção errada: penaliza
            engine.currentText = (text[pos] === ch) ? (text.slice(0,pos) + text.slice(pos+1)) : text.replace(ch,"");
          ...
          // feedback
          scoreFloat(delta, el);
          toast?.(rule ? "✅ Pontuação removida" : "⚠️ Remoção incorreta (-1)");
...
function onPlainClick(el){
  ...
  function renderFinalMessages(){
    ...
    for (let i=0;i<3;i++){
      ...
      const perfect = applyRules(snap.before, snap.rules);
      const userText = String((snap.after ?? snap.before) || "");
      ...
      box.innerHTML = `
        <div class="final-msg">
          <div class="muted" style="font-size:12px; margin-bottom:6px">Seu texto</div>
    ...
    try{
      ...
      if (engine.challenge !== 0 && stNow?.level){
        levelSnapshots[engine.levelIndex] = {
          ...
          rules: Array.isArray(engine.currentRules) ? engine.currentRules : (engine.getState?.()?.level?.rules || []),
          fixes: engine.getFixLog?.() || [],
