...
// ===== Avatar local (gratuito) =====
function getLocalAvatar(uid){
  if (!uid) return "";
  return localStorage.getItem(`avatarDataUid:${uid}`) || "";
}
function setLocalAvatarForCurrentUser(dataUrl){
  try{
    const u = getAuth().currentUser;
    if (!u) return;
    localStorage.setItem(`avatarDataUid:${u.uid}`, dataUrl);
    // Atualiza previews se existirem
    const img = document.querySelector("#accountAvatarImg");
    if (img && dataUrl) img.src = dataUrl;
    const badge = document.querySelector("#miniAvatarImg");
    if (badge && dataUrl) badge.src = dataUrl;
  }catch{}
}
...
onClick: () => {
  ...
  async function openAccountPanel(){
    ...
    const photoURL = (localStorage.getItem(`avatarDataUid:${currentUser.uid}`) || currentUser.photoURL || '');
    openModal({
      title: "👤 Minha conta",
      bodyHTML: `
    ...
    if (fileEl){
      fileEl.addEventListener('change', async () => {
        ...
        try{
          ...
          await syncAvatarToRanking(dataUrl);
          if (preview){
  ...
  async function syncAvatarToRanking(dataUrl){
    // We avoid Firebase Auth photoURL for base64 (too long).
    // Keep a compact avatar stored locally and mirror to Firestore ranking doc (free).
    try{
      if(app?.ranking?.setMyAvatar){
        await app.ranking.setMyPhotoURL(dataUrl);
      }
    }catch(e){
      console.warn('[auth] syncAvatarToRanking falhou', e);
    }
  }

  async function updateMyPhotoURL(dataUrl){
  const currentUser = getAuth().currentUser;
  if (!currentUser) throw new Error("not-auth");
  const em = (currentUser.email || "").trim().toLowerCase();
  if (!em) throw new Error("no-email");

  // usamos o hash do e-mail para chave local (não expõe e-mail em texto)
  const uid = currentUser.uid;
  const emailHash = await sha256Hex(em);

  // Ajusta para um tamanho seguro (dataURL grande quebra updateProfile/photoURL)
  const img = new Image();
  ...
  // JPEG reduz bastante o tamanho.
  const finalURL = c.toDataURL("image/jpeg", 0.85);

  // Guardamos localmente (gratuito). Ranking público continua com ícones/iniciais.
  localStorage.setItem(`avatarTypeUid:${uid}`, "upload");
  localStorage.setItem(`avatarDataUid:${uid}`, finalURL);
  // cache por emailHash (usado no ranking)
  try {
    const k = "mission_avatar_by_emailhash_v1";
    const map = JSON.parse(localStorage.getItem(k) || "{}");
    map[emailHash] = finalURL;
    localStorage.setItem(k, JSON.stringify(map));
  } catch {}

  // Atualiza preview no perfil sem mexer no Firebase Auth (evita auth/invalid-profile-attribute).
  setLocalAvatarForCurrentUser(finalURL);

  // Opcional: se existir ranking local, re-renderiza.
