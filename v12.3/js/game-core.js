...
export function bootGame(app){
  ...
  // Marcações de erros (para manter vermelho mesmo após re-render)
  // Obs: não bloqueia novas tentativas (o usuário pode errar de novo),
  // mas mantém feedback visual.
  engine.wrongRuleIds = engine.wrongRuleIds || new Set();
      ...
      if (bestRule?.clickTokenOnly && bestRule?.token && best.text.includes(bestRule.token)){
        ...
        for (const ch of best.text){
          if (ch === tok){
            ...
            // feedback persistente de erro (vermelho)
            if (engine.wrongRuleIds?.has?.(bestRule.id)){
              cspan.classList.add("wrong");
            }
        ...
        buttons: [
          ...
          {label:"Remover", onClick: () => {
            ...
            try{
              ...
              try{ engine.wrongRuleIds?.delete?.(rule.id); }catch(e){}
  ...
  function onTokenClick(el, rule){
    ...
    function doFastFix(){
      try{
        ...
        // se estava marcado como erro, limpa
        try{ engine.wrongRuleIds?.delete?.(rule.id); }catch(e){}
    ...
    openModal({
      ...
      buttons:[
        ...
        {label: isRemove ? "Remover" : "Confirmar", onClick: () => {
          ...
          if (!ok){
            // marca em vermelho (sem bloquear novas tentativas)
            try{ engine.wrongRuleIds?.add?.(rule.id); }catch(e){}
            if (tokenEl){
