    ...
    return `
      <div class="rank-list">
        ...
        ${rows.map((d,i)=>{
          ...
          const av = (d && typeof d.avatar === 'string' && d.avatar) ? d.avatar : getAvatarForHash(d?.emailHash);
          const avHtml = av
  ...
  app.ranking = {
    ...
    setMyAvatar: async (photoURL) => {
      try{
        ...
        // firestore (shared)
        const db = getDb();
        const ref = doc(db, 'rankingByEmail', h);
        await setDoc(ref, { avatar: photoURL || '', updatedAt: serverTimestamp() }, { merge: true });
      }catch(e){
