...
export function bootGame(app){
  ...
  dom.missionSpecialHomeBtn?.addEventListener("click", () => {
    ...
    if (!app.auth?.isLogged?.()){
      if (app.auth?.openAuthGate) { app.auth.openAuthGate(); }
      else { openModal({ title:"🔒 Login necessário", bodyHTML:`<p>Faça login para acessar a <b>Missão Especial</b>.</p>`, buttons:[{label:"Ok", variant:"ghost", onClick: closeModal}] }); }
      return;
    }
    if (!app.auth?.isLogged?.()){
      if (app.auth?.openAuthGate) { app.auth.openAuthGate(); }
