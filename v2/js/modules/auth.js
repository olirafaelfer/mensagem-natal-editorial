    ...
    return `
      ...
      <div data-pane="login">
        ...
        <div class="auth-actions">
          ...
          <button class="btn ghost" data-action="google" type="button" title="Em breve">Entrar com Google (em breve)</button>
...
try {
  const cred = if (!isValidEmail(email)) { throw new Error("E-mail inválido."); }
      await createUserWithEmailAndPassword(auth, String(emailClean||"").trim(), pass);
...
// Mostra o botão na UI; implementação completa pode ser ativada depois
export function setupGoogleButton(container){
  if (!container) return;
  if (container.querySelector('[data-google-login]')) return;
  const btn = document.createElement('button');
  btn.type = 'button';
  btn.className = 'btn secondary';
  btn.textContent = 'Entrar com Google (em breve)';
  btn.setAttribute('data-google-login','1');
  btn.addEventListener('click', () => {
    alert('Login Google será ativado em uma etapa posterior.');
  });
